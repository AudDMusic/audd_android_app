...
public class RecyclerListView extends RecyclerView {
    ...
    private class FastScroll extends View {
        ...
        public FastScroll(Context context) {
            ...
            letterPaint.setTextSize(Screen.dp(45));
            for (int a = 0; a < 8; a++) {
                radii[a] = Screen.dp(44);
            }

            scrollX = false ? Screen.dp(10) : Screen.dp(117);
            updateColors();
        }

        private void updateColors() {
            int inactive = Color.GRAY; //Theme.getColor(Theme.key_fastScrollInactive);
            int active = Color.GRAY; // Theme.getColor(Theme.key_fastScrollActive);
        ...
        public boolean onTouchEvent(MotionEvent event) {
            switch (event.getAction()) {
                case MotionEvent.ACTION_DOWN:
                    float x = event.getX();
                    lastY = event.getY();
                    float currectY = (float) Math.ceil((getMeasuredHeight() - Screen.dp(24 + 30)) * progress) + Screen.dp(12);
                    if (false && x > Screen.dp(25) || !false && x < Screen.dp(107) || lastY < currectY || lastY > currectY + Screen.dp(30)) {
                        return false;
                    }
                ...
                case MotionEvent.ACTION_MOVE:
                    ...
                    float minY = Screen.dp(12) + startDy;
                    float maxY = getMeasuredHeight() - Screen.dp(12 + 30) + startDy;
                    if (newY < minY) {
                        newY = minY;
                    } else if (newY > maxY) {
                        newY = maxY;
                    ...
                    float dy = newY - lastY;
                    lastY = newY;
                    progress += dy / (getMeasuredHeight() - Screen.dp(24 + 30));
                    if (progress < 0) {
                        progress = 0;
                    } else if (progress > 1) {
                        progress = 1;
                    ...
                    if (adapter instanceof FastScrollAdapter) {
                        ...
                        if (newLetter == null) {
                            ...
                            if (letterLayout.getLineCount() > 0) {
                                ...
                                if (false) {
                                    textX = Screen.dp(10) + (Screen.dp(88) - letterLayout.getLineWidth(0)) / 2 - letterLayout.getLineLeft(0);
                                } else {
                                    textX = (Screen.dp(88) - letterLayout.getLineWidth(0)) / 2 - letterLayout.getLineLeft(0);
                                }
                                textY = (Screen.dp(88) - letterLayout.getHeight()) / 2;
                            }
                        }
                    }
                }
        ...
        protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
            setMeasuredDimension(Screen.dp(132), MeasureSpec.getSize(heightMeasureSpec));
        }
        ...
        protected void onDraw(Canvas canvas) {
            ...
            int y = (int) Math.ceil((getMeasuredHeight() - Screen.dp(24 + 30)) * progress);
            rect.set(scrollX, Screen.dp(12) + y, scrollX + Screen.dp(5), Screen.dp(12 + 30) + y);
            canvas.drawRoundRect(rect, Screen.dp(2), Screen.dp(2), paint);
            if ((pressed || bubbleProgress != 0)) {
                paint.setAlpha((int) (255 * bubbleProgress));
                int progressY = y + Screen.dp(30);
                y -= Screen.dp(46);
                float diff = 0;
                if (y <= Screen.dp(12)) {
                    diff = Screen.dp(12) - y;
                ...
                float raduisTop;
                float raduisBottom;
                canvas.translate(Screen.dp(10), y);
                if (diff <= Screen.dp(29)) {
                    raduisTop = Screen.dp(44);
                    raduisBottom = Screen.dp(4) + (diff / Screen.dp(29)) * Screen.dp(40);
                } else {
                    diff -= Screen.dp(29);
                    raduisBottom = Screen.dp(44);
                    raduisTop = Screen.dp(4) + (1.0f - diff / Screen.dp(29)) * Screen.dp(40);
                }
                if (false && (radii[0] != raduisTop || radii[6] != raduisBottom) || !false && (radii[2] != raduisTop || radii[4] != raduisBottom)) {
                    if (false) {
                    ...
                    path.reset();
                    rect.set(false ? Screen.dp(10) : 0, 0, Screen.dp(false ? 98 : 88), Screen.dp(88));
            ...
            gestureDetector = new GestureDetector(context, new GestureDetector.SimpleOnGestureListener() {
                ...
                public boolean onSingleTapUp(MotionEvent e) {
                    if (currentChildView != null && onItemClickListener != null) {
                        ...
                        Application.runOnUIThread(clickRunnable = new Runnable() {
                            @Override
                            public void run() {
                        ...
                        if (selectChildRunnable != null) {
                            ...
                            Application.cancelRunOnUIThread(selectChildRunnable);
        ...
        public boolean onInterceptTouchEvent(RecyclerView view, MotionEvent event) {
            ...
            if (action == MotionEvent.ACTION_DOWN || action == MotionEvent.ACTION_POINTER_DOWN) {
                if (!interceptedByChild && currentChildView != null) {
                    ...
                    Application.runOnUIThread(selectChildRunnable, ViewConfiguration.getTapTimeout());
    ...
    public void cancelClickRunnables(boolean uncheck) {
        ...
        if (clickRunnable != null) {
            Application.cancelRunOnUIThread(clickRunnable);
            clickRunnable = null;
        }
        interceptedByChild = false;
    }
    ...
    public RecyclerListView(Context context) {
        super(context);
        selectorDrawable = new ColorDrawable(Color.GRAY); // Theme.getSelectorDrawable(false);
    ...
    protected void onMeasure(int widthSpec, int heightSpec) {
        ...
        if (fastScroll != null) {
            fastScroll.measure(MeasureSpec.makeMeasureSpec(Screen.dp(132), MeasureSpec.EXACTLY), MeasureSpec.makeMeasureSpec(getMeasuredHeight(), MeasureSpec.EXACTLY));
        }
    }
        ...
        try {
            ...
            if (sectionsType == 1) {
                ...
                for (int a = 0; a < headers.size(); a++) {
                    ...
                    canvas.translate(false ? getWidth() - header.getWidth() : 0, top);
                ...
                canvas.translate(false ? getWidth() - pinnedHeader.getWidth() : 0, top);
    ...
    public static abstract class GridAdapter<VH> extends RecyclerView.Adapter {
